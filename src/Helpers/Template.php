...
class Template
{
    ...
    {
        return plugin_dir_path(ZIMRATE_FILE) . 'src' . DIRECTORY_SEPARATOR . 'Views' . DIRECTORY_SEPARATOR . ltrim($path, '\\/');
    }
    ...
    {
        return plugin_dir_url(ZIMRATE_FILE) . 'src' . self::$URL_SEPARATOR . 'Views' . self::$URL_SEPARATOR . ltrim($url, '\\/');
    }
    ...
    /**
     * Get template file path (legacy support)
     *
     * @param string $path Path to template file relative to Views directory
     * @return string Full path to template file
     *
     * @author Richard Muvirimi <richard@tyganeutronics.com>
     * @since 1.0.0
     */
    public static function get_template_path(string $path): string
    {
        return self::get_views_path($path);
    }

    /**
     * Get template file URL (legacy support)
     *
     * @param string $url Path to template file relative to Views directory
     * @return string Full URL to template file
     *
     * @author Richard Muvirimi <richard@tyganeutronics.com>
     * @since 1.0.0
     */
    public static function get_template_url(string $url): string
    {
        return self::get_views_url($url);
    }
}
