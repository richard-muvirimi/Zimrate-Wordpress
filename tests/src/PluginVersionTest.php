...
class PluginVersionTest extends TestCase
{
    ...
    public function testPluginVersions(): void
    {
        ...
        self::assertTrue(version_compare(ZIMRATE_VERSION, $rmTxtVersion, "="), "readme.txt Plugin version does not match the stable version : " . ZIMRATE_VERSION);
        self::assertTrue(version_compare(ZIMRATE_VERSION, $rmMdVersion, "="), "readme.md Plugin version does not match the stable tag version : " . ZIMRATE_VERSION);
        self::assertTrue(version_compare(ZIMRATE_VERSION, $pluginVersion, "="), "Plugin version does not match the stable tag version : " . ZIMRATE_VERSION);
    }
	...
	private function getPluginVariable(string $name):string{
		...
		preg_match("/".$name."\s*:\s*(.*)$/mi", file_get_contents(ZIMRATE_FILE), $matches);
	...
	private function getReadMeMdVariable(string $name):string{
		...
		preg_match("/".$name."\s?:.+\s(.*)$/miU", file_get_contents(dirname(ZIMRATE_FILE) . DIRECTORY_SEPARATOR.$readme), $matches);
	...
	private function getReadMeTxtVariable(string $name):string{
		...
		preg_match("/".$name."\s?:\s?(.*)$/mi", file_get_contents(dirname(ZIMRATE_FILE) . DIRECTORY_SEPARATOR.$readme), $matches);
